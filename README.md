...
| No. | コマンド                  | 説明                                                                   |
|----:|:--------------------------|:-----------------------------------------------------------------------|
|   1 | git cherry-pick main      | mainブランチの内容を現在のブランチに取り込む(コミットが生成される)     |
|   2 | git cherry-pick `-n` main | mainブランチの内容を現在のブランチに取り込む(コミットが生成され`ない`) |
|   3 | git rebase <ブランチ名>   | HEADの根本を, 指定したブランチの根本に移動する.                        |
|   4 | git rebase --continue     |                                                                        |
|   5 | git rebase --abort        | rebaseを中止する.                                                      |
...
rebaseはGit履歴を変更するので, rebaseするブランチを既にリモートにpushしている場合は通常の git push は機能しない. その場合は`-f`オプションを付けて強制pushする必要がある(ローカルリポジトリの状態(履歴)を優先してpushする). 複数人で共同開発している場合は注意してrebaseを使用する必要がある.  
OSSへの貢献を行う場合はrebaseがよく使われる. これはプルリクを送る側がコンフリクトを解消しておくため.  
実務ではチームのルールに従うべし. (git rebaseの代わりにgit mergeを使うことができる)
